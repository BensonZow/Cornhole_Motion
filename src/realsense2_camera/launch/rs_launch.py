...
def launch_setup(context, params, param_name_suffix=''):
    ...
    return [
        LogInfo(msg='[rs_launch_timing] seq=1 phase=launch_setup_start'),
        LogInfo(
            msg=(
                f'[rs_launch_timing] seq=2 phase=post_param_checks '
                f'segment_s={t1 - t0:.4f}'
            )
        ),
        LogInfo(
            msg=(
                f'[rs_launch_timing] seq=3 phase=post_lifecycle_yaml '
                f'segment_s={t2 - t1:.4f}'
            )
        ),
        LogInfo(
            msg=(
                f'[rs_launch_timing] seq=4 phase=pre_return '
                f'node_build_s={t3 - t2:.4f} total_s={t3 - t0:.4f}'
            )
        ),
